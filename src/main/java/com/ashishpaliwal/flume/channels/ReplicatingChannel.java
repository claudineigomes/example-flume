...
/**
 * A Replicating Channel based on a distributed Queue mechanism
 */
public class ReplicatingChannel extends BasicChannelSemantics {

  private static Logger LOGGER = LoggerFactory.getLogger(ReplicatingChannel.class);
...
//            if(!queue.offer(putList.removeFirst())) {
            if(!hazelcastQueue.offer(putList.removeFirst())) {
      ...
      channelCounter.setChannelSize(hazelcastQueue.size());
    }
    ...
    protected void doRollback() {
      int takes = takeList.size();
      synchronized(queueLock) {
        Preconditions.checkState(hazelcastQueue.remainingCapacity() >= takeList.size(), "Not enough space in memory channel " +
                "queue to rollback takes. This should never happen, please report");
        while(!takeList.isEmpty()) {
//          queue.addFirst(takeList.removeLast());
          //TODO Look into this logic, hazelcast doesn't support addFirst
          hazelcastQueue.add(takeList.removeLast());
        }
        putList.clear();
      }
      bytesRemaining.release(putByteCounter);
...
//  private LinkedBlockingDeque<Event> queue;
  ...
  @GuardedBy(value = "queueLock")
  private BlockingQueue<Event> hazelcastQueue;
  ...
  public void configure(Context context) {
    ...
    try {
      capacity = context.getInteger(ChannelConstants.CAPACITY, defaultCapacity);
    ...
    try {
      transCapacity = context.getInteger(ChannelConstants.TRANSACTION_CAPACITY, defaultTransCapacity);
    } catch(NumberFormatException e) {
    ...
    try {
      byteCapacityBufferPercentage = context.getInteger(ChannelConstants.BYTE_CAPACITY_BUFFER_PERCENTAGE, defaultByteCapacityBufferPercentage);
    } catch(NumberFormatException e) {
      byteCapacityBufferPercentage = defaultByteCapacityBufferPercentage;
    }

    try {
      byteCapacity = (int)((context.getLong(ChannelConstants.BYTE_CAPACITY, defaultByteCapacity) * (1 - byteCapacityBufferPercentage * .01 )) /byteCapacitySlotSize);
      if (byteCapacity < 1) {
    ...
    try {
      keepAlive = context.getInteger(ChannelConstants.KEEP_ALIVE, defaultKeepAlive);
    } catch(NumberFormatException e) {
      keepAlive = defaultKeepAlive;
    }

    // Add handling for Queue
    ClientConfig hazelcastConfig = new ClientConfig();
    hazelcastConfig.getGroupConfig().setName(context.getString(ChannelConstants.HAZELCAST_USER_KEY,
                                              ChannelConstants.DEFAULT_HAZELCAST_USER))
                                    .setPassword(context.getString(ChannelConstants.HAZELCAST_PASS_KEY,
                                            ChannelConstants.DEFAULT_HAZELCAST_PASSWORD));
    hazelcastConfig.getNetworkConfig().addAddress(context.getString(ChannelConstants.HAZELCAST_SERVERS_KEY,
                                                                    ChannelConstants.DEFAULT_HAZELCAST_SERVER));

    SerializerConfig serializerConfig = new SerializerConfig();
    serializerConfig.setImplementation(new KryoEventSerializer()).setTypeClass(Event.class);
    ...
    hazelcastQueue = hazelcastInstance.getQueue(context.getString(ChannelConstants.HAZELCAST_QUEUE_NAME_KEY,
            ChannelConstants.DEFAULT_QUEUE_NAME));
...
//  }
  ...
  public synchronized void start() {
    ...
    channelCounter.setChannelCapacity(Long.valueOf(
            hazelcastQueue.size() + hazelcastQueue.remainingCapacity()));
    super.start();
  }
